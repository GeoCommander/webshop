...
class ProductFiltersBuilder:
    ...
    def get_attribute_filters(self):
        ...
        for name, values in attribute_value_map.items():
            # Use the sort_values function to sort the attribute values
            sorted_values = sort_values(values)
            out.append(frappe._dict(name=name, item_attribute_values=sorted_values))
        return out
...
def sort_values(values):
    try:
        # Attempt to convert all values to floats for sorting
        return sorted(values, key=lambda x: float(x))
    except ValueError:
        # Fallback to normal sort if any value can't be converted to float
